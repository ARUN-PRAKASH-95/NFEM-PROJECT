...
for time,tau in enumerate(t):
    ...
    stress=np.zeros_like(coordinate)
        ...
        for i in range(len(coordinate)-1):
            ...
            def material_routine(coordinate):
                ...
                if Trial_eq - yield_stress < 0:
                    stress[i]=np.asscalar(Trial_stress[1])
                    print('ELASTIC',tau,i)
                    ...
                    stress[i]=np.asscalar(new_stress[1])
    ...
    ax[0].plot(coordinate,stress)
    ax[1].plot(coordinate,Global_displacement)
